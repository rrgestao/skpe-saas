...
`IMPLEMENTADO — 2026-09-13`: a criação automática e atômica `Iniciativa → Projeto SK-PE → binding → Jornada → auditoria` permanece como fundação. A migration `20260913155108_govern_skpe_sparkoop_project_leader_bootstrap.sql` endurece o bootstrap para criar somente `owner` inicial resolvido por **Pessoa vinculada à SPARKOOP + papel organizacional ativo `project_leader`**, eliminando o bootstrap legado `owner+sponsor+facilitator` baseado apenas no usuário criador. Também disponibiliza `reconcile_skpe_project_opening_owner(...)`, que na Reunião de Abertura encerra exclusivamente o owner inicial SPARKOOP registrado pelo próprio bootstrap e cria o responsável da Organização com trilha auditável.

`GATE_RUNTIME — PASS`: Ricardo Rodrigues (`ricardo.rodrigues@sparkoop.com`) foi definido por decisão humana como **Líder de Projeto pela SPARKOOP**, possui membership SPARKOOP ativo, vínculo Pessoa→SPARKOOP ativo e atribuição organizacional ativa ao papel `project_leader` desde 2026-09-13. A migration foi aplicada no Supabase operacional e as funções/privilegios foram confirmados por readback. O histórico existente, inclusive COOTAQUARA, não foi reescrito automaticamente.
