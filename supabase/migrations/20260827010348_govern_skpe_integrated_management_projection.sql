...
-- 17-B.5F.3C.6K-PROPOSED
-- Integrated management projection for one SK-PE project.
-- Read-only composition of existing governed sources of truth.
-- No automatic mutation, no duplicated schedule, no duplicated Kanban,
-- no economic roll-up persistence, no capacity normalization.
...
revoke all on function public.get_skpe_project_management_projection(uuid, uuid, date) from public;
revoke all on function public.get_skpe_project_management_projection(uuid, uuid, date) from anon;

grant execute on function public.get_skpe_project_management_projection(uuid, uuid, date) to authenticated;
grant execute on function public.get_skpe_project_management_projection(uuid, uuid, date) to service_role;
