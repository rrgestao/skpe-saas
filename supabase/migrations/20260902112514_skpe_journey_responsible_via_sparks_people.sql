create or replace function public.get_skpe_journey_temporal_read_model(target_organization_id uuid, target_project_id uuid default null::uuid, target_as_of_date date default null::date)
returns table(
  ...
  current_plan_approved_at timestamp with time zone,
  ...
  current_forecast_activated_at timestamp with time zone,
...
begin
  ...
  select
    ...
    coalesce(
      nullif(trim(responsible_person.full_name), ''),
      nullif(trim(responsible_person.preferred_name), ''),
      responsible_person.primary_email
    ),
  ...
  left join lateral (
    ...
    join public.sparks_organization_people sop
     ...
     and sop.organization_id = p.organization_id
     and sop.status = 'active'
     and (sop.start_date is null or sop.start_date <= p.reference_date)
     and (sop.end_date is null or sop.end_date >= p.reference_date)
    where sp.profile_user_id = i.responsible_user_id
      and sp.person_status = 'active'
    order by
      sop.is_primary_relationship desc,
      sop.start_date desc nulls last,
      sop.id
    limit 1
  ) responsible_person on true
  left join lateral (
    select v.*
