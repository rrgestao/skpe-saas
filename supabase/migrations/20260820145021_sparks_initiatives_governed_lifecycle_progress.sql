...
begin
  ...
  if not (
    ...
    or (v_initiative.status = 'on_hold' and v_target_status in ('in_progress','blocked','cancelled'))
...
comment on function public.transition_sparks_initiative_lifecycle(uuid, text, text) is
  'Executa transição governada do lifecycle organizacional transversal de uma iniciativa SPARKs, sem sincronização automática com status metodológicos de módulos especializados.';


create or replace function public.update_sparks_initiative_execution(
  target_initiative_id uuid,
...
begin
  ...
  if v_initiative.status in ('completed','cancelled') then
    raise exception 'A execução não pode ser alterada após o encerramento da iniciativa (%).',
      v_initiative.status
      using errcode = '22023';
  end if;

  v_progress := coalesce(target_progress, v_initiative.progress);
  v_health_status := case
    when target_health_status is null then v_initiative.health_status
    else lower(trim(target_health_status))
  end;
  ...
  if v_health_status not in ('not_assessed','on_track','attention','critical','completed') then
    raise exception 'Status de saúde da execução inválido: %.', target_health_status
      using errcode = '22023';
  end if;

  if v_initiative.status in ('proposed','under_analysis','approved','planned')
     and v_progress > 0 then
    raise exception 'Progresso acima de zero exige lifecycle em execução, suspenso ou bloqueado.'
      using errcode = '22023';
  end if;

  if v_progress = 100 then
    raise exception 'Progresso 100 deve ser registrado pela transição governada para completed.'
      using errcode = '22023';
  end if;

  if v_health_status = 'completed' then
    raise exception 'health_status completed deve ser definido somente pela transição governada para completed.'
      using errcode = '22023';
  end if;

  if v_progress is not distinct from v_initiative.progress
     and v_health_status is not distinct from v_initiative.health_status then
    raise exception 'Nenhuma alteração de progresso ou saúde foi identificada.'
      using errcode = '22023';
  end if;
  ...
  v_before := jsonb_build_object(
    ...
    'progress', v_initiative.progress,
    'health_status', v_initiative.health_status,
    'last_update_at', v_initiative.last_update_at
  );
...
revoke all
on function public.update_sparks_initiative_execution(uuid, numeric, text, text)
from public, anon;
...
comment on function public.update_sparks_initiative_execution(uuid, numeric, text, text) is
  'Atualiza de forma governada o progresso organizacional e a saúde da execução de uma iniciativa SPARKs, sem alterar lifecycle automaticamente e sem sincronizar status ou progresso metodológicos de módulos especializados.';
