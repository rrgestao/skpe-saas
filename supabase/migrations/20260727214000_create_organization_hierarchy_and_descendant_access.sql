  ...
  select distinct
    ...
    case
      when exists (
        ...
        where om.organization_id = o.id and om.user_id = auth.uid() and om.status = 'active'
      ) then 'direct_membership'
      else 'descendant_policy'
    end as access_origin
  from public.organizations o
  where public.can_access_descendant_organization(o.id, target_module_code, requested_action)
  order by coalesce(o.trade_name, o.legal_name, o.code);
$$;
