    ...
    case
      ...
      then jsonb_build_object(
        ...
        'colors', jsonb_build_object(
          'primary', coalesce(o.visual_identity_metadata#>>'{colors,primary}', '#002C2F'),
          'secondary', coalesce(o.visual_identity_metadata#>>'{colors,secondary}', '#01877A'),
          'accent', coalesce(o.visual_identity_metadata#>>'{colors,accent}', '#FF4F0C'),
          'support_1', coalesce(o.visual_identity_metadata#>>'{colors,support_1}', '#F27F64'),
          'support_2', coalesce(o.visual_identity_metadata#>>'{colors,support_2}', '#4EA797'),
          'support_3', coalesce(o.visual_identity_metadata#>>'{colors,support_3}', '#000000'),
...
create or replace function public.update_sparks_organization_visual_identity(
  ...
  target_support_1_color text,
  target_support_2_color text,
  target_support_3_color text,
...
declare
  ...
  normalized_support_1 text := upper(trim(coalesce(target_support_1_color, '')));
  normalized_support_2 text := upper(trim(coalesce(target_support_2_color, '')));
  normalized_support_3 text := upper(trim(coalesce(target_support_3_color, '')));
...
begin
  ...
  if normalized_mode = 'sparks_default' then
    ...
    if normalized_primary !~ '^#[0-9A-F]{6}$'
      ...
      or normalized_support_1 !~ '^#[0-9A-F]{6}$'
      or normalized_support_2 !~ '^#[0-9A-F]{6}$'
      or normalized_support_3 !~ '^#[0-9A-F]{6}$' then
      raise exception 'As seis cores devem usar o formato hexadecimal #RRGGBB.';
    end if;
    ...
    next_metadata := jsonb_build_object(
      ...
      'colors', jsonb_build_object(
        ...
        'support_1', normalized_support_1,
        'support_2', normalized_support_2,
        'support_3', normalized_support_3,
