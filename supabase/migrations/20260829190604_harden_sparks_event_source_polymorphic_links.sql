...
as $function$
  with normalized as (
    select
      upper(trim(coalesce(target_source_module_code, ''))) as module_code,
      lower(trim(coalesce(target_source_entity_type, ''))) as entity_type
  )
  select
    ...
    and (
      (
        n.module_code = ''
        and n.entity_type = ''
        and target_source_entity_id is null
        and public.can_manage_organization(target_organization_id)
      )
      or (
        n.module_code = 'SK-PE'
        and n.entity_type = 'skpe_journey_item'
      ...
      or (
        n.entity_type = 'sparks_initiative'
        and target_source_entity_id is not null
        and exists (
          select 1
          ...
          where i.id = target_source_entity_id
            ...
            and coalesce(upper(trim(i.source_module_code)), '') = n.module_code
      ...
      or (
        n.entity_type = 'sparks_initiative_action'
        and target_source_entity_id is not null
        and exists (
          select 1
          ...
          where a.id = target_source_entity_id
            ...
            and coalesce(upper(trim(a.source_module_code)), '') = n.module_code
            and public.can_manage_sparks_initiatives(
              target_organization_id,
...
comment on function public.can_manage_sparks_event_source(uuid, text, text, uuid) is
  'Valida de forma fechada os vinculos polimorficos autorizados de eventos. Evento nativo exige ausencia de fonte e gestao da organizacao; fontes vinculadas exigem tipo reconhecido, existencia do objeto e autorizacao da autoridade correspondente.';
